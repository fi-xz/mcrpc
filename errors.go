...
// ErrNotConnected is returned when a call is attempted on a client whose
// connection was never established or has already been closed.
...
func (c *MCRPCClient) call(ctx context.Context, method string, params, result any) error {
	if c == nil || c.JSONRPCConn == nil {
		return &Error{Method: method, err: ErrNotConnected}
	}

	err := c.JSONRPCConn.Call(ctx, method, params, result)
	if err == nil {
		return nil
	}
